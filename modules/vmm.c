...
void vmm_entry(void)
{
	...
	/* set the interrupt vector to Realtime domain */
	...
	/* switch to realtime domain */
	entry(&eparam);

	spin_unlock_irqrestore(&init_lock, flags);

	/* we now switched to virtual IRQ but the hardware IRQ is disabled
	 * before entering realtime domain. Therefore, we have to enabled. */
	asm volatile("cpsie i":::"memory", "cc");
}

static int vmm_loader(const char *filename)
{
	...
	printk("loading realtime domain:%s", filename);
	/* FIXME: we should not need this actually. But, Linux would hang
	 * without doing this at present. */
	...
	set_fs(get_ds());
	while (file_sz > 0) {
		len = vfs_read(flp, (void __user __force *) buf_ptr,
		               BUFF_SZ, &pos);
		file_sz -= len;
		buf_ptr += len;
	}
